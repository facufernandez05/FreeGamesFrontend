...
import { useFetchFreeGamesByCategory } from '@/hooks/useFetchAllGames'
import { useState } from 'react'

export default function GamesByCategory ({ params }) {
  const [sort, setSort] = useState('relevance')
  const { category } = params
  const { favs } = useFav()
  const { games } = useFetchFreeGamesByCategory(sort, 'all', category)

  return (
    <main className='flex flex-col justify-center max-w-5xl gap-4 p-4 mx-auto'>
      <h2 className='text-4xl font-bold'>Top {category.toUpperCase()} Free Games in 2023!</h2>
